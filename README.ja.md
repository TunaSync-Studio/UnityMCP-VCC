...
- **プラグイン側がリスナー** → MCP クライアントは複数同時接続可能 (2.6.8 から
  1 エディタあたり最大 16 接続)。
...
開く前に完了します。この 2 本 (読み取り専用の `vcc_project` も、マシン上の
全プロジェクト名を列挙するため) は配信モードでは他の破壊系ツールと同様
ロックされます。
...
- **`vrc_upload`**: `dry_run:true` (検証) と実アバター公開経路は実プロジェクトで
  ...
  (TTL 30 分・1 回の arm で 1 試行) — で、v2.6.7 からは **arm ファイルを
  エディタプラグイン側でも検査**するため、このサーバを迂回するクライアント
  でも無人公開はできません。リポジトリのチェックアウト無しで arm する方法:
  `docs/INSTALL.ja.md` の「実 VRChat アップロードの arm」参照。注意:
  2.6.7/2.6.8 の npm サーバはプラグイン側の検査より先に arm ファイルを消費して
  いたため、実アップロードは必ず拒否されていました ("arm file not found")。
  2.6.8 以降の修正で arm は試行が終わるまで残るようになりました。修正後の
  実公開経路の実射検証はまだです。
- **配信モード**: `UNITY_MCP_STREAM_MODE=1` でプロジェクトを書き換える系・
  公開系ツールをロックし、ツール出力とレシピリソースのユーザーパスを
  マスクします (画面共有セッション用) — `docs/STREAMING.md`。
