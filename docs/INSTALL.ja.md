...
> **要件 / 状態**: Windows・Unity **2022.3** (2022.3.22f1 で検証)・Node.js 20.19+。
...
30 分で失効し、次の実アップロード試行が終わった時点で消費されます
(アップロードジョブ開始時にプラグインも同じファイルを再確認するため、
試行中は残っています)。**AI に作らせないでください** — 人間がボタンに
触れたこと自体が、このゲートの意味です。`UNITY_MCP_ARM_FILE` で場所を
変える場合は、Unity エディタのプロセスにも同じ値を設定してください。
